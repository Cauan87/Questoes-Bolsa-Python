def palindrome(texto):
    '''Faça uma função que verifique se o texto passado é palíndrome,
    isto é, se é igual quando lido de trás pra frente.'''
    texto_sem_espaco = ""
    for sinal in texto:
        if sinal.isalpha():  # Verifica se o caractere é uma letra
            texto_sem_espaco += sinal.lower()
    
    texto_invertido = texto_sem_espaco[::-1]

    return texto_sem_espaco == texto_invertido
...
def troca_caixa(texto):
    ...
    for letras in texto:
        if letras.lower() in vogais:
            texto_final += letras.upper()
        else:
            texto_final += letras.lower()
...
def numeros_sortudos(limite_inferior=1, limite_superior=100000):
    ...
    for numero in range(limite_inferior, limite_superior + 1):
        if numero_sortudo(numero):
            count += 1
    return count

def numero_sortudo(numero):
    numero_str = str(numero)
    return '2' in numero_str and '7' not in numero_str
