...
def maiores_13(idades, alturas):
    ...
    if not idades or not alturas:
        return []

    media_alturas = sum(alturas) / len(alturas)
    alturas_selecionadas = []

    for i in range(len(idades)):
        if idades[i] > 13 and alturas[i] < media_alturas:
            alturas_selecionadas.append(alturas[i])

    return alturas_selecionadas
...
def media_saltos_lista(saltos):
    ...
    if not saltos or len(saltos) < 3:
        return 0
    
    saltos_ordenados = sorted(saltos)
    saltos_ordenados = saltos_ordenados[1:-1]  # Remove o melhor e pior saltos
...
def lista_de_primos(inicio, fim):
    ...
    primos = [num for num in range(inicio, fim + 1) if is_primo(num)]
    return primos
...
def Fibonacci(n):
    ...
    if n <= 1:
        ...
        while len(sequencia_de_fibonacci) < n:
            proximo_num = sequencia_de_fibonacci[-1] + sequencia_de_fibonacci[-2]
            sequencia_de_fibonacci.append(proximo_num)
...
def altera_salarios(salarios):
    ...
    novos_salarios_arredondados = [round(salario, 2) for salario in novos_salarios]
    return novos_salarios_arredondados
